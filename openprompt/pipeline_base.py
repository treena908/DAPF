...
class PromptDataLoader(object):
    ...
    def tokenize(self) -> None:
        ...
        """
        if self.template is None: #template and verbalizer None when we are  not wrapping input, just tensoring raw input
            self.wrapped_dataset=self.raw_dataset
        ...
        #     decoded=self.tokenizer.decode(self.tokenizer.convert_tokens_to_ids(
...
class PromptModel(nn.Module):
    ...
    def forward(self, batch: Union[Dict, InputFeatures]) -> torch.Tensor:
        ...
        # outputs,soft_outputs = self.template.post_processing_outputs(outputs)
...
class PromptForClassification(nn.Module):
    ...
    def forward(self, batch: Union[Dict, InputFeatures]) -> torch.Tensor:
        ...
        """
        # outputs,soft_outputs = self.prompt_model(batch)
        outputs= self.prompt_model(batch)
        # soft_embed=soft_outputs[0] #firt soft embed
        # print(soft_embed)
