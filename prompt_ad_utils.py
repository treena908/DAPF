...
def read_input_text_len_control(df, sample_size=-1, max_len=128, token_cut=True):
    ...
    if sample_size==-1:
        output_df = texts_df[["filename","id", "joined_all_par_trans", "ad"]]
        ...
        list_output_dfs = [texts_df[["filename","id", "joined_all_par_trans", "ad"]].iloc[i1:i2, :].reset_index(drop=True) for [i1, i2] in indexes_list]
        return list_output_dfs

def read_input_no_len_control(df, model, sample_size=-1, max_len=512,domain_col='', save_trans=None,label_col=None):
    print('no_len_control')    
    if save_trans is not None:
        ...
        def pre_tokenizing(text):
            ...
            print(len(text))
            print(type(text))
            temp=window_token_based_cut(text, tokenizer, max_len)[0]
            # print('hi')
            # print(len(temp))
            # print(type(temp))
            return temp
        ...
        if len(domain_col)>2:
            domain_len=len(df.loc[0,domain_col].split()) #length of the domain info that will be added with the main text
            max_len=max_len-domain_len-4 #-4 is for the classification prompt to be added
        # df.joined_all_par_trans = df.joined_all_par_trans.apply(lambda x: pre_tokenizing(x))

        df.text = df.text.apply(lambda x: pre_tokenizing(x))

        # texts_df = df[pre_tokened_series.apply(lambda x: True if len(x) == 1 else False)]
    if 'text' not in df.columns.tolist():
        col_to_add = ["filename", "id", "joined_all_par_trans", "ad", "domain", "domain1", "domain2"]
        if label_col in ['pTau_Positivity','AB42_AB40Positivity','tTau_AB42Positivity','MOCA_impairment']:
            col_to_add.append(label_col)
        output_df = df[col_to_add]


    else:
        col_to_add = ["filename", "id", "text", "ad", "domain", "domain1", "domain2"]
        if label_col in ['pTau_Positivity', 'AB42_AB40Positivity', 'tTau_AB42Positivity', 'MOCA_impairment']:
            col_to_add.append(label_col)
        output_df = df[col_to_add]
    ...
    if sample_size==-1:
        ...
        # output_df = df[["filename", "id", "text", "ad","domain"]]  # "age_group",
        if save_trans is not None:
            output_df.to_csv(save_trans)
        return output_df.reset_index(drop=True)
        raise NotImplemented
