...
class FewShotSampler(object):
    ...
    def __init__(self,
        ...
        if also_sample_dev:
            if num_examples_total_dev is not None and num_examples_per_label_dev is not None:
                ...
                print("specify neither num_examples_total_dev nor num_examples_per_label_dev, set to default (equal to train set setting).")
    ...
    def _sample(self,
        ...
        if num_examples_per_label is not None:
            ...
            # assert hasattr(data[0], 'ad'), "sample by label requires the data has a 'label' attribute."
            # labels = [x.ad for x in data]
            labels=data.ad.values.tolist()
        ...
        if sample_twice:
            ...
            if self.num_examples_per_label_dev is not None:
                ...
                # assert hasattr(data[0], 'ad'), "sample by label requires the data has a 'label' attribute."
                remain_labels = [x.ad for idx, x in enumerate(data) if idx not in selected_set]
            ...
            if isinstance(data, pd.DataFrame):
                sampled_data= data[data['id'].isin(selected_ids)]
                df = sampled_data[sampled_data.columns.tolist()].copy()
                return df
    ...
    def sample_per_label(self, indices: List, labels, num_examples_per_label):
        ...
        for label, ids in ids_per_label.items():
            ...
            if len(tmp) < num_examples_per_label:
                ...
                print("Not enough examples of label {} can be sampled".format(label))
        ...
        print("Selected examples {}".format(selected_ids))
        return selected_ids
